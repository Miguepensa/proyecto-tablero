...
type QuickFilter = "TOTAL" | "ABIERTAS" | "VENCIDAS" | "BLOQUEADAS";
type SearchParams = Record<string, string | string[] | undefined>;

const KPI_COLORS: Record<QuickFilter, string> = {
  TOTAL: "#00AAFF",
  ABIERTAS: "#36FF7D",
  VENCIDAS: "#FFC100",
  BLOQUEADAS: "#FF0027",
...
function statusTone(status?: string | null): Tone {
  ...
  if (["TERMINADO", "COMPLETADO", "COMPLETED", "PUESTA_EN_MARCHA"].includes(normalized)) {
    return "green";
  }
...
function getFilterParam(value: string | string[] | undefined): QuickFilter {
...
function filterHref(
  params: SearchParams,
  key: "projectFilter" | "storyFilter" | "reqFilter",
  value: QuickFilter
) {
  ...
  for (const [paramKey, paramValue] of Object.entries(params)) {
    ...
    search.set(paramKey, Array.isArray(paramValue) ? paramValue[0] : paramValue);
  }
...
function MiniStat({
  ...
  const className = `rounded-2xl p-3 text-left transition ${
    active
      ? "border bg-white shadow-sm ring-2"
      : href
        ? "border border-transparent bg-slate-50 hover:bg-white hover:shadow-sm"
        : "bg-slate-50"
  }`;
...
export default async function DashboardPage({
  ...
  const projectFilter = getFilterParam(params.projectFilter);
  const storyFilter = getFilterParam(params.storyFilter);
  const reqFilter = getFilterParam(params.reqFilter);
  ...
  const recentStories = stories.filter((story) => matchesQuickFilter(story, storyFilter)).slice(0, 6);
            ...
            <BoardPanel title="Proyectos" subtitle="Avance general, estados y proyectos recientes." href="/projects" actionLabel="Ver proyectos">
              <div className="grid grid-cols-2 gap-3">
                <MiniStat
                  label="Total"
                  value={totalProjects}
                  href={filterHref(params, "projectFilter", "TOTAL")}
                  active={projectFilter === "TOTAL"}
                  color={KPI_COLORS.TOTAL}
                />
                <MiniStat
                  label="Abiertos"
                  value={activeProjects}
                  href={filterHref(params, "projectFilter", "ABIERTAS")}
                  active={projectFilter === "ABIERTAS"}
                  color={KPI_COLORS.ABIERTAS}
                />
                <MiniStat
                  label="Vencidos"
                  value={overdueProjects}
                  href={filterHref(params, "projectFilter", "VENCIDAS")}
                  active={projectFilter === "VENCIDAS"}
                  color={KPI_COLORS.VENCIDAS}
                />
                <MiniStat
                  label="Bloq."
                  value={blockedProjects}
                  href={filterHref(params, "projectFilter", "BLOQUEADAS")}
                  active={projectFilter === "BLOQUEADAS"}
                  color={KPI_COLORS.BLOQUEADAS}
                />
              </div>
            ...
            <BoardPanel title="Historias de usuario" subtitle="Flujo por fase, asignación y últimos registros." href="/stories" actionLabel="Ver historias">
              <div className="grid grid-cols-2 gap-3">
                <MiniStat
                  label="Total"
                  value={totalStories}
                  href={filterHref(params, "storyFilter", "TOTAL")}
                  active={storyFilter === "TOTAL"}
                  color={KPI_COLORS.TOTAL}
                />
                <MiniStat
                  label="Abiertas"
                  value={activeStories}
                  href={filterHref(params, "storyFilter", "ABIERTAS")}
                  active={storyFilter === "ABIERTAS"}
                  color={KPI_COLORS.ABIERTAS}
                />
                <MiniStat
                  label="Vencidas"
                  value={overdueStories}
                  href={filterHref(params, "storyFilter", "VENCIDAS")}
                  active={storyFilter === "VENCIDAS"}
                  color={KPI_COLORS.VENCIDAS}
                />
                <MiniStat
                  label="Bloq."
                  value={blockedStories}
                  href={filterHref(params, "storyFilter", "BLOQUEADAS")}
                  active={storyFilter === "BLOQUEADAS"}
                  color={KPI_COLORS.BLOQUEADAS}
                />
              </div>
            ...
            <BoardPanel title="Requerimientos" subtitle="Resumen tomado de actividades/tareas registradas." href="/requirements" actionLabel="Ver requerimientos">
              <div className="grid grid-cols-2 gap-3">
                <MiniStat
                  ...
                  href={filterHref(params, "reqFilter", "TOTAL")}
                  active={reqFilter === "TOTAL"}
                  color={KPI_COLORS.TOTAL}
                />
                <MiniStat
                  label="Abiertas"
                  value={activeRequisitions}
                  href={filterHref(params, "reqFilter", "ABIERTAS")}
                  active={reqFilter === "ABIERTAS"}
                  color={KPI_COLORS.ABIERTAS}
                />
                <MiniStat
                  label="Vencidas"
                  value={overdueRequisitions}
                  href={filterHref(params, "reqFilter", "VENCIDAS")}
                  active={reqFilter === "VENCIDAS"}
                  color={KPI_COLORS.VENCIDAS}
                />
                <MiniStat
                  label="Bloq."
                  value={blockedRequisitions}
                  href={filterHref(params, "reqFilter", "BLOQUEADAS")}
                  active={reqFilter === "BLOQUEADAS"}
                  color={KPI_COLORS.BLOQUEADAS}
                />
              </div>
